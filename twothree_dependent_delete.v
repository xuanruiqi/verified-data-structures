...
Section search_tree.
  ...
  Proof.
    ...
    rewrite in_sorted_left. rewrite IHl //= /is_search_tree. apply: cat_sorted_l.
    exact: H. exact: H. exact: Hlt.
    ...
    case: ifP => Hlt_b //=. rewrite /is_search_tree /= => H.
    rewrite in_sorted_right. rewrite in_sorted_left. rewrite IHm //= /is_search_tree.
    rewrite -cat_rcons in H. apply: cat_sorted_l. apply: cat_sorted_r. exact: H.
    apply: cat_sorted_r. rewrite -cat_rcons in H. exact: H. exact. exact.
    by rewrite ltnNge leq_eqVlt Hlt_a Heq_a.
    case: ifP => Heq_b //=. rewrite /is_search_tree /= => H.
    rewrite (eqP Heq_b). by rewrite mem_cat in_cons mem_cat in_cons eq_refl !orbT.
    rewrite /is_search_tree /= => H. rewrite in_sorted_right.
    rewrite in_sorted_right. rewrite IHr //= /is_search_tree.
    apply: sorted_rest. apply: cat_sorted_r. apply: cat_sorted_r.
    rewrite -!cat_cons in H. exact: H. apply: sorted_rest. apply: cat_sorted_r.
    exact: H. by rewrite ltnNge leq_eqVlt Hlt_b Heq_b. exact.
    by rewrite ltnNge leq_eqVlt Hlt_a Heq_a.
  Qed.
